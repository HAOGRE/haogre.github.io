---
author: "HAOGRE"
pubDatetime: 2026-07-30T10:30:00.000Z
title: "Afterword: The Professional Ethics of FDEs"
featured: false
draft: false
lang: en
translationKey: "09-后记-FDE的职业道德"
tags:
  - "musings"
  - "AI"
  - "business"
description: "FDEs handle sensitive data and consequential decisions. This afterword sets six ethical boundaries for the profession."
---

Every book about methodology must end by addressing one question: once you have mastered the method, where are the boundaries? The subject of this book is heavier than that of most methodology books. An FDE does not merely hold ordinary technical knowledge—**they hold the deepest secrets of a client organization, along with an ever-growing power to make decisions on people’s behalf**.

The nature of FDE work determines what you will see. To deliver well, you need access to the client’s most honest operating data—including the ugly parts. You need to understand the organization’s power map—including who is incompetent and who is losing influence. You need to enter core business processes—including workarounds that may sit in gray areas. The client opens all of this to you based on a simple agreement: you are there to help. That trust is the foundation of the FDE model, and destroying it takes only one boundary crossed.

In my view, FDE ethics have at least six red lines. I offer them here as a shared commitment to everyone in the profession.

![Image 1](/Users/haogre/.gemini/antigravity-cli/brain/d4dffd42-6b6d-4063-b3a3-ad5b7fc7e2fe/fde_rise_2_1785725012734.jpg)

**First, data sovereignty belongs to the client.** Not a single byte of data you see on a client site should end up where it does not belong—not in AI training data unless the contract explicitly permits it, not in case-study material unless the client gives written consent, and not as gossip for your next job. Data minimization is not merely a technical standard; it is professional conduct. If you do not need to look at something, do not look. If it can be anonymized, anonymize it.

**Second, report results honestly, including bad news.** In a performance-based model, the greatest ethical risk is disguising outcomes—packaging “the system went live” as “value was realized,” and correlation as causation. The value-measurement system described in Chapter 6 can be either the most honest tool or the most sophisticated machine for manufacturing lies. The difference lies only in the people using it. An FDE’s foundation is the willingness to be tested. When the results are poor, you must present them with the same willingness.

**Third, do not create dependence or sell fear.** This industry contains two subtle forms of harm. One is deliberately turning a system into a black box so the client can never do without you. The other is amplifying the fear that “you will die without AI” to close a deal. Gartner predicts that by 2028, 70% of enterprises will be forced to abandon frontline-deployment-led approaches because of costs and hollowed-out skills—a warning to the entire industry. A healthy FDE model should be one where **you leave when the work is done**: knowledge is transferred to the client, and capability stays with the client team. **Make clients stronger, not addicted.**

![Image 2](/Users/haogre/.gemini/antigravity-cli/brain/d4dffd42-6b6d-4063-b3a3-ad5b7fc7e2fe/fde_rise_3_1785725020908.jpg)

**Fourth, take the people being replaced seriously.** In many settings, the systems FDEs deliver really will replace part of someone’s work. This book discusses many techniques of “change management,” but there is an ethical dimension beyond technique: do not celebrate efficiency in front of the people being replaced; do not describe people as “cost items” in a proposal without offering them a path forward; and do not pretend not to see whose destiny you are changing. Technological neutrality is a myth. The deployer’s choices are reality.

**Fifth, say no to things the client asks for but should not do.** You will encounter requests that sit on the edge of compliance: “Help us make employee-behavior monitoring more granular,” or “The compliance status of this data is a little unclear, but let’s take it on first.” The “French waiter” metaphor carries its deepest meaning here. True professionalism is not satisfying every client request; it is having the courage to guide the client toward what is genuinely good for them and harmless to the world. The confidence to say no comes from having other clients on your books. Ethics, therefore, have always been connected to the business model.

**Sixth, remember that you represent technology itself.** To many clients, you are the first human face they see when they encounter AI. Every exaggeration you make spends down the industry’s credibility; every promise you keep deposits credit into it. As AI becomes more deeply embedded in how society operates, deployers become the final translators between technology and everyday human life—and everyone pays the cost when that translation is distorted.

![Image 3](/Users/haogre/.gemini/antigravity-cli/brain/d4dffd42-6b6d-4063-b3a3-ad5b7fc7e2fe/ch2_1_1785725049130.jpg)

Palantir itself is deeply controversial. Its history of serving intelligence and military organizations has made many people wary of everything associated with it, including the FDE model. My extensive use of its methodology in this book does not mean I endorse all of its choices of clients. On the contrary, precisely because it operates in such sensitive fields, the disciplines embedded in its engineering culture—permissions, auditing, and need-to-know access—are especially worth learning from. **The greater the deployer’s capabilities, the more urgently they must write the boundaries before the method.**

FDE is a young profession, and no one has written its code of conduct yet. I hope this book becomes its first boundary marker.

## Acknowledgments

Thank you to former Palantir employees including Bob McGrew, Barry, Ted Mabry, and Nabil Qureshi for sharing your memories and writing publicly—you turned the methodology of a “mysterious company” into public knowledge. Thank you to the hosts of podcasts such as YC Lightcone and Latent Space; your questions preserved so much firsthand experience. Thank you for the research and reporting from a16z, MIT’s NANDA laboratory, The New Stack, and CIO.com. And thank you to the FDE pioneers in China who have written their thoughts on their websites—you made it possible for this conversation in the Chinese-speaking world to begin somewhere other than zero.

Thank you to everyone willing to read this far. May every road you build have someone to walk it; may the road still be there when you reach the end.
