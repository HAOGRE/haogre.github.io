---
author: "HAOGRE"
pubDatetime: 2026-06-01T03:34:02.802Z
title: "Typora 快捷键速查与自定义指南"
featured: false
draft: false
tags:
  - "随笔"
description: "Typora 常用快捷键一览表，涵盖文件、编辑、段落、格式、视图等操作，并介绍 macOS 和 Windows/Linux 下自定义快捷键的方法。"
---

## 摘要

你可以使用快捷键快速插入或修改样式，或执行 Typora 支持的其他操作。

你可以在菜单栏中每个菜单项的右侧找到对应的快捷键。

## 常用快捷键

### 自动补全

在 macOS 上，按 Esc 键可以打开内联数学公式的内联预览、表情符号的自动补全等。
...
| 全选 | Ctrl + A | Command + A |
| 选择行/句子/选择行（表格中） | Ctrl + L | Command + L |
| 删除行（表格中） | Ctrl + Shift + Backspace | Command + Shift + Backspace |
| 选择样式范围/选择单元格（表格中） | Ctrl + E | Command + E |
...
| 放大 | Ctrl + Shift + = | *(不支持)* |
| 缩小 | Ctrl + Shift + - | *(不支持)* |
| 切换打开的文档 | Ctrl + Tab | Command + ` |
| 切换开发者工具 | Shift + F12 | - |

## 更改快捷键

用户可以在 Typora 的菜单栏中设置或重新分配命令的快捷键。本文将介绍如何操作。

### macOS

在 macOS 上，我们使用系统功能来自定义 Typora 的键绑定。

1. 打开 `系统偏好设置` → `键盘`，选择 `快捷键` 标签页。在左侧面板中选择 `App 快捷键`。

   ![Snip20160814_1](/uploads/2026/06/01/1001-shortcut-keys/Snip20160814_1.png)

2. 点击“+”按钮，弹出对话框。在 `应用程序` 字段中选择 `Typora.app`，然后输入要添加的菜单命令的确切名称，并输入要分配的快捷键组合。

   例如，假设你想添加/更改固定 Typora 窗口的快捷键，可以按如下方式填写对话框：

   ![Snip20160814_5](/uploads/2026/06/01/1001-shortcut-keys/Snip20160814_5.png)

   然后 Command+Shift+P 将成为菜单命令“Always On Top”的快捷键组合。

### Windows / Linux

1. 在 Typora 中打开 `菜单` → `偏好设置`，然后点击“打开高级设置”。

   ![sshot-1](/uploads/2026/06/01/1001-shortcut-keys/sshot-1.png)

2. 在打开的“文件资源管理器”中打开并编辑 `conf.user.json`。如果不存在该文件，请创建一个。

3. 设置或添加一个表示键绑定的 JSON 对象，例如：

   ![Snip20160814_7](/uploads/2026/06/01/1001-shortcut-keys/Snip20160814_7.png)

4. 重启 Typora。新的键绑定将生效。

   ![sshot-2](/uploads/2026/06/01/1001-shortcut-keys/sshot-2.png)

你可以在原生窗口样式的菜单栏中设置或更改菜单项的快捷键。

#### 问：Ubuntu 上快捷键不起作用

有关此问题的解决方案，请参阅：

- https://help.ubuntu.com/community/KeyboardShortcuts 和
